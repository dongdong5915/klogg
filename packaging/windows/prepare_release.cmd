...
set "TBB_BIN="
for /d %%D in ("%KLOGG_WORKSPACE%\%KLOGG_BUILD_ROOT%\msvc_*") do if exist "%%~fD\tbb12.dll" if not defined TBB_BIN set "TBB_BIN=%%~fD"
if not defined TBB_BIN (
    echo Could not find tbb12.dll in %KLOGG_BUILD_ROOT%
    exit /b 1
)
xcopy "%TBB_BIN%\tbb12.dll" "%KLOGG_WORKSPACE%\release\" /y
if errorlevel 1 exit /b 1
if exist "%TBB_BIN%\tbb12.pdb" xcopy "%TBB_BIN%\tbb12.pdb" "%KLOGG_WORKSPACE%\release\" /y
